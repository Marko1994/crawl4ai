...
def test_a_recovered_retry_is_logged_but_not_as_a_failure(monkeypatch, caplog):
    """Recoveries must be visible, and must not look like blocks.

    Without this, the retry is unmeasurable: a run showing zero `dns_failure`
    lines is indistinguishable from a run where DNS simply never faltered, so
    there is no way to demonstrate the retry is doing anything. Log the
    recovery separately, and do NOT emit `cause=dns_failure` - that counter has
    to keep meaning "a lookup was given up on".
    """
    resolver = _FlakyResolver(fail_times=1)
    monkeypatch.setattr(socket, "getaddrinfo", resolver.getaddrinfo)

    with caplog.at_level(logging.INFO, logger="egress_broker"):
        resolve_and_pin("https://bearwoodconcepts.com")

    text = caplog.text
    assert "cause=dns_failure" not in text, "a recovered lookup is not a failure"
    assert "dns_retry" in text, "the recovery must be observable"
    assert "bearwoodconcepts.com" in text


def test_an_exhausted_lookup_still_logs_a_dns_failure(monkeypatch, caplog):
    resolver = _FlakyResolver(fail_times=99)
    monkeypatch.setattr(socket, "getaddrinfo", resolver.getaddrinfo)

    with caplog.at_level(logging.INFO, logger="egress_broker"):
        with pytest.raises(EgressBlocked):
            resolve_and_pin("https://gone.invalid")

    assert "cause=dns_failure" in caplog.text
